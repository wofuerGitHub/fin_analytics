...
Structure:
    ...
    5.1   normalize parameters, like e.g. 1/opt_perf*perf + 1/opt_vola*vola + \
            1/target_dividend*dividend + 1/target_bookvalue*bookvalue + 1/target_eps*eps
...
import scipy.optimize as opt                                    # interpolation
...
my_portfolio = pd.merge(reference, my_portfolio[['symbol', 'all']], \
    how = "left", on = ['symbol'])
my_portfolio['all'] = my_portfolio['all'].fillna(0)
my_portfolio = my_portfolio[my_portfolio['all'] != 0]   # remove all = 0 assets
...
for index, row in my_portfolio.iterrows():
    ...
    my_portfolio.loc[index, 'per'] = ts.iloc[-1]['per']             # last per to my_portf.
    my_portfolio.loc[index, 'last_close'] = ts.iloc[-1]['close']    # last close to my_p.
    ts.drop(columns = ['per'], inplace = True)
    ts_normalized = standardizeTimeSerie(ts, 'endDate-1year', 'lastBDay')   # one-year
    ts_portfolio[row['symbol']] = ts_normalized['close']*row['all']
my_portfolio.set_index('symbol', inplace=True) # perf, vola, sr per equity

"""
ts_portfolio:
            ANDR.VI   BEI.DE   BMW.DE     EXSA.DE  LEN.DE       OMV.DE       STR.VI   VAS.DE
date                                                                                                 
2023-05-24  7235.80  7643.20  5927.20  15880.2655  4725.6  4224.350000  7422.700000  5266.80
2023-05-23  7235.80  7643.20  5927.20  15880.2655  4725.6  4224.350000  7422.700000  5266.80
...             ...      ...      ...         ...     ...          ...          ...      ...
2022-05-25  5147.34  6466.80  4870.10  15534.0725  5563.8  5005.199854  8168.949841  4222.68
2022-05-24  5106.20  6485.84  4822.98  15428.7870  5478.0  5005.199854  8288.350298  4167.24
...
for index, row in my_portfolio.iterrows():  # iterate over portfolio and add earning price ratio
    ...
    try:
        ...
        query_result = mysql_db.get_mysql_data(connection_to_source, source, \
            ...
            filter_symbol = row['symbol_fundamental'], order_by = "date", \
...
my_portfolio['start_weight'] = ts_portfolio.iloc[0]/ts_total.iloc[0]    # share of portf. on first date
my_portfolio['end_weight'] = ts_portfolio.iloc[-1]/ts_total.iloc[-1]    # share of portf. on last date
...
def portfolio(weights, mean_returns, cov_matrix):
    """
    Calculate portfolio return, variance and std
    """
...
daily_ret = calc_returns(ts_portfolio, resample=None, ret_type="log")   # calc. daily returns
mean_returns, cov_matrix = calc_returns_stats(daily_ret)    # calc. mean returns & cov.-matrix

# print(mean_returns)
# print(cov_matrix)

# functions to minimize
def neg_sharpe_ratio(weights, mean_returns, cov_matrix, risk_free_rate=0):  # calculate negative SR
...
def optimize_sharpe_ratio(mean_returns, cov_matrix, risk_free_rate=0, w_bounds=(0,1)):
    """
    Function finds the portfolio weights which minimize the negative sharpe ratio
    """
    ...
    if result['success']:
        ...
        return(opt_sharpe, opt_weights, opt_return.item()*252, \
            opt_variance.item()*252, opt_std.item()*(252**0.5))
    else:
...
opt_sharpe, opt_weights, opt_return, opt_variance, opt_std = optimize_sharpe_ratio(
                            mean_returns, cov_matrix, risk_free_rate=0, \
                            w_bounds=(share_min, share_max))
my_portfolio['opt_sr'] = opt_weights

# --- optimize portfolio for linear in addition to earnings, bookval., ... -> latest asset-info

# functions to minimize
def neg_performance_linear_weight(weights, mean_returns, cov_matrix, linear, \
    linear_weight = 1, risk_free_rate = 0):  # calculate negative sharpe ratio
    """
    Negative sharpe ratio 
    """
    portfolio_return, portfolio_var, portfolio_std = portfolio(weights, mean_returns, cov_matrix)
    opt = portfolio_return*100*252 + np.dot(weights, linear)*linear_weight
    return -opt

def optimize_performance_linear_weight(mean_returns, cov_matrix, linear, linear_weight, \
    risk_free_rate=0, w_bounds=(0,1)):  # optimize for performance and epr
    """
    Function finds the portfolio weights which minimize the negative sharpe ratio
    """
    init_guess = np.array([1/len(mean_returns) for _ in range(len(mean_returns))])
...
# S&P long term perf ~ 7% / epr ~ 0.066 --> factor 7 / 0.066 = 105 = long term equality
opt_sharpe, opt_weights, opt_return, opt_variance, opt_std = optimize_performance_linear_weight(
                            mean_returns, cov_matrix, my_portfolio['epr'], 155, \
                            risk_free_rate = 0, w_bounds=(share_min, share_max))
my_portfolio['opt_perf_epr'] = opt_weights

# max shapre portfolio & bpr
# S&P long term perf ~ 7% / bpr ~ 1/2.81 = 0.35 --> factor 7 / 0.35 = 20 = long term equality
opt_sharpe, opt_weights, opt_return, opt_variance, opt_std = optimize_performance_linear_weight(
                            mean_returns, cov_matrix, my_portfolio['bpr'], 30, \
                            risk_free_rate = 0, w_bounds=(share_min, share_max))
...
for x in variation:
    performance, variance, standard_deviation = portfolio(my_portfolio[x].values, \
                                                    mean_returns, cov_matrix)
    append_dict = {'name':x, 'perf':performance*100*252, \
        'vola': standard_deviation*100*(252**0.5), \
        'sr':performance*(252**.5)/standard_deviation, \
        'epr': np.dot(my_portfolio[x], my_portfolio['epr']), \
        'per':1/np.dot(my_portfolio[x], my_portfolio['epr']), \
        'bpr': np.dot(my_portfolio[x], my_portfolio['bpr']), \
        'pbr':1/np.dot(my_portfolio[x], my_portfolio['bpr'])}
...
try:
    ...
    mysql_db.put_dataframe_to_mysql(connection_to_target, target, \
                                    my_portfolio, CONFIG[METHOD]["pk_target_portfolio"], \
                                    update_timestamp = True)
    connection_to_target.close()
except:
    writeLog(CONFIG['file']['log'], 'Error writing to ' \
        + CONFIG[METHOD]["table_target_portfolio"]+' - no update', id = log_id)

try:
    connection_to_target = sql_engine.connect()
    target = mysql_db.get_metatable(sql_engine, CONFIG[METHOD]["table_target_portfolio_opt"])
    mysql_db.put_dataframe_to_mysql(connection_to_target, target, \
                                    my_portfolio_opt, CONFIG[METHOD]["pk_target_portfolio_opt"], \
                                    update_timestamp = True)
    connection_to_target.close()
except:
    writeLog(CONFIG['file']['log'], 'Error writing to ' \
        + CONFIG[METHOD]["table_target_portfolio_opt"]+' - no update', id = log_id)
